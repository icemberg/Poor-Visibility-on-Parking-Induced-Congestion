...
PHASE7_DIRS = [
    Path("content/phase7_outputs_2"),
    Path("phase7_outputs_2"),
    Path("/content/phase7_outputs_2"),
]
...
def load_forecast_bundle():
    for d in PHASE7_DIRS:
        if d.exists():
            f = load_if_exists(d / "phase7_next_week_forecast_ranked.csv")
            if f is not None:
                return ensure_label_column(derive_coords(f)), d / "phase7_next_week_forecast_ranked.csv"
    return None, None


def recommended_shift_from_row(row):
    peak = row.get("peak_hour", np.nan)
    if pd.isna(peak):
        peak = row.get("recommended_peak_hour", np.nan)
    if pd.isna(peak):
        ccs = safe_float(row.get("ccs_score", 0.0), 0.0)
        if ccs >= 80:
            return "Morning", "08:00–11:00"
        if ccs >= 60:
    ...
    if 10 <= peak < 14:
        return "Midday", "10:00–13:00"
    if 14 <= peak < 19:
        return "Evening", "15:00–18:00"
    return "Night", "20:00–23:00"


def compute_dispatch_priority(df: pd.DataFrame, forecast_df: Optional[pd.DataFrame] = None):
    work = df.copy()
    if len(work) == 0:
        return work
    ...
    if "ccs_score" not in work.columns:
        work["ccs_score"] = 0.0
    if "growth_pct_change" not in work.columns:
        work["growth_pct_change"] = 0.0
    if "criticality_factor" not in work.columns:
        work["criticality_factor"] = 1.0
    if "risk_band" not in work.columns:
        work["risk_band"] = "Watch"

    work["ccs_score"] = pd.to_numeric(work["ccs_score"], errors="coerce").fillna(0.0)
    work["growth_pct_change"] = pd.to_numeric(work["growth_pct_change"], errors="coerce").fillna(0.0)
    work["criticality_factor"] = pd.to_numeric(work["criticality_factor"], errors="coerce").fillna(1.0)

    if forecast_df is not None and len(forecast_df):
        f = forecast_df.copy()
        
        # Ensure 'cluster_key' exists in f
        if "cluster_key" not in f.columns:
            if "st_dbscan_cluster_id" in f.columns:
                f["cluster_key"] = f["st_dbscan_cluster_id"]
            elif "cluster_id" in f.columns:
                f["cluster_key"] = f["cluster_id"]
            else:
                f["cluster_key"] = f.index
        
        # Force string type for robust merging
        f["cluster_key"] = f["cluster_key"].astype(str)
        
        if "probability_of_escalation" not in f.columns:
            f["probability_of_escalation"] = 0.0
        if "predicted_next_week_ccs_score" not in f.columns:
            f["predicted_next_week_ccs_score"] = f.get("ccs_score", 0.0)
        
        f = f[["cluster_key", "probability_of_escalation", "predicted_next_week_ccs_score", "future_risk_band"]].drop_duplicates("cluster_key")
        
        # Ensure 'cluster_key' exists in work
        if "cluster_key" not in work.columns:
            if "st_dbscan_cluster_id" in work.columns:
                work["cluster_key"] = work["st_dbscan_cluster_id"]
            elif "cluster_id" in work.columns:
                work["cluster_key"] = work["cluster_id"]
            else:
                work["cluster_key"] = work.index
        
        # Force string type
        work["cluster_key"] = work["cluster_key"].astype(str)
        
        work = work.merge(f, on="cluster_key", how="left")
    else:
        work["probability_of_escalation"] = np.nan
        work["predicted_next_week_ccs_score"] = np.nan
        work["future_risk_band"] = np.nan

    work["probability_of_escalation"] = pd.to_numeric(work["probability_of_escalation"], errors="coerce").fillna(0.0)
    work["predicted_next_week_ccs_score"] = pd.to_numeric(work["predicted_next_week_ccs_score"], errors="coerce").fillna(work["ccs_score"])

    work["dispatch_priority_score"] = (
        0.38 * minmax(work["ccs_score"]) +
        0.18 * minmax(work["predicted_next_week_ccs_score"]) +
        0.18 * minmax(work["probability_of_escalation"]) +
        0.14 * minmax(work["growth_pct_change"]) +
        0.12 * minmax(work["criticality_factor"])
    ) * 100.0
    return work.sort_values("dispatch_priority_score", ascending=False).reset_index(drop=True)
...
def haversine_km(lat1, lon1, lat2, lon2):
    ...
    return 6371.0 * (2 * math.asin(min(1.0, math.sqrt(max(a, 0.0)))))


def road_speed_kmh(road_class, default=24.0):
    speeds = {
        "motorway": 55.0,
        "trunk": 50.0,
        "primary": 40.0,
    ...
    return speeds.get(str(road_class).strip().lower(), default)


def build_route_plan(plan_df: pd.DataFrame, depot_lat: float = BENGALURU_CENTER[0], depot_lon: float = BENGALURU_CENTER[1]):
    cols = [
        "route_order", "cluster_id", "cluster_label_display", "risk_band", "lat", "lon",
        "leg_km", "leg_eta_min", "cumulative_km", "cumulative_eta_min",
        "recommended_shift", "recommended_time_window", "unit_action"
    ]
    ...
    remaining = work.reset_index(drop=True).copy()
    remaining["lat"] = pd.to_numeric(remaining["lat"], errors="coerce")
    remaining["lon"] = pd.to_numeric(remaining["lon"], errors="coerce")
    remaining = remaining.dropna(subset=["lat", "lon"]).copy()

    current_lat, current_lon = float(depot_lat), float(depot_lon)
    cumulative_km = 0.0
    cumulative_eta = 0.0
    ordered = []

    while len(remaining):
        remaining["_dist_km"] = remaining.apply(lambda r: haversine_km(current_lat, current_lon, r["lat"], r["lon"]), axis=1)
        if "phase8_priority_score" in remaining.columns:
            remaining["_route_key"] = remaining["_dist_km"] - 0.02 * pd.to_numeric(remaining["phase8_priority_score"], errors="coerce").fillna(0.0)
        else:
            remaining["_route_key"] = remaining["_dist_km"]
        idx = remaining["_route_key"].idxmin()
        row = remaining.loc[idx].copy()

        leg_km = float(haversine_km(current_lat, current_lon, row["lat"], row["lon"]))
        speed = road_speed_kmh(row.get("road_class", "road"))
        if pd.notna(row.get("probability_of_escalation", np.nan)):
            speed *= max(0.7, 1.0 - min(0.25, float(row.get("probability_of_escalation", 0.0)) * 0.15))
        if str(row.get("risk_band", "")).lower() in {"critical", "high"}:
            speed *= 0.9
        leg_eta = max(3.0, (leg_km / max(speed, 5.0)) * 60.0)

        cumulative_km += leg_km
        cumulative_eta += leg_eta

        ordered.append({
            "route_order": len(ordered) + 1,
            "cluster_id": row.get("cluster_id", row.get("st_dbscan_cluster_id", np.nan)),
            "cluster_label_display": row.get("cluster_label_display", row.get("cluster_label", "")),
            "risk_band": row.get("risk_band", "Watch"),
            "lat": float(row["lat"]),
            "lon": float(row["lon"]),
            "leg_km": leg_km,
            "leg_eta_min": leg_eta,
            "cumulative_km": cumulative_km,
            "cumulative_eta_min": cumulative_eta,
            "recommended_shift": row.get("recommended_shift", "Morning"),
            "recommended_time_window": row.get("recommended_time_window", "08:00–11:00"),
            "unit_action": row.get("unit_action", "Monitor"),
        })

        current_lat, current_lon = float(row["lat"]), float(row["lon"])
        remaining = remaining.drop(index=idx).reset_index(drop=True)

    return pd.DataFrame(ordered)


def build_offender_actions(offenders_df: Optional[pd.DataFrame]):
    cols = [
        "vehicle_number", "total_violations", "unique_hotspots", "dominant_vehicle_type",
        "first_seen", "last_seen", "offender_tier", "recommended_action", "watchlist_flag"
    ]
    if offenders_df is None or len(offenders_df) == 0 or "vehicle_number" not in offenders_df.columns:
        return pd.DataFrame(columns=cols)

    work = offenders_df.copy()
    for c in ["total_violations", "unique_hotspots"]:
        if c not in work.columns:
            work[c] = 0
        work[c] = pd.to_numeric(work[c], errors="coerce").fillna(0)

    if "dominant_vehicle_type" not in work.columns:
        work["dominant_vehicle_type"] = "UNKNOWN"
    if "first_seen" not in work.columns:
        work["first_seen"] = pd.NaT
    if "last_seen" not in work.columns:
        work["last_seen"] = pd.NaT

    work = work.sort_values(["total_violations", "unique_hotspots"], ascending=[False, False]).copy()
    work["offender_tier"] = np.select(
        [work["total_violations"] >= 20, work["total_violations"] >= 10, work["total_violations"] >= 5],
        ["Critical", "High", "Moderate"],
        default="Watch",
    )
    work["recommended_action"] = np.select(
        [work["offender_tier"].eq("Critical"), work["offender_tier"].eq("High"), work["offender_tier"].eq("Moderate")],
        ["Immediate intercept / tow watchlist", "Focused LPR surveillance + patrol alert", "Targeted repeat-offender monitoring"],
        default="Routine watch",
    )
    work["watchlist_flag"] = work["total_violations"] >= 5
    return work.reset_index(drop=True)


def build_phase8_engine(dispatch_df: pd.DataFrame, offenders_df: Optional[pd.DataFrame]):
    if dispatch_df is None or len(dispatch_df) == 0:
        empty_plan = pd.DataFrame()
        return empty_plan, empty_plan, build_offender_actions(offenders_df), pd.DataFrame([{
            "planned_hotspots": 0,
            "tow_ready_hotspots": 0,
            "critical_hotspots": 0,
            "high_hotspots": 0,
            "mean_dispatch_priority_score": np.nan,
            "mean_phase8_priority_score": np.nan,
            "mean_probability_escalation": np.nan,
            "mean_patrol_allocation": np.nan,
        }])

    work = dispatch_df.copy()
    work = ensure_label_column(work)
    work = derive_coords(work)

    defaults = {
        "dispatch_priority_score": 0.0,
        "probability_of_escalation": 0.0,
        "predicted_next_week_ccs_score": work.get("ccs_score", 0.0),
        "future_risk_band": work.get("risk_band", "Watch"),
        "forecast_recommendation": "Routine monitoring",
    ...
    for c in ["dispatch_priority_score", "probability_of_escalation", "predicted_next_week_ccs_score",
              "growth_pct_change", "criticality_factor", "ccs_score", "delay_minutes_per_vehicle",
              "repeat_vehicle_count_2plus", "chronic_vehicle_count_5plus", "layer_b_alert_flag",
              "layer_d_alert_flag"]:
        work[c] = pd.to_numeric(work[c], errors="coerce").fillna(0.0)

    work["risk_band"] = work.get("risk_band", "Watch").fillna("Watch").astype(str)
    work["future_risk_band"] = work.get("future_risk_band", work["risk_band"]).fillna(work["risk_band"]).astype(str)

    offender_pressure = (
        0.6 * minmax(work["chronic_vehicle_count_5plus"]) +
        0.4 * minmax(work["repeat_vehicle_count_2plus"])
    )

    forecast_pressure = minmax(work["predicted_next_week_ccs_score"])
    escalation_pressure = minmax(work["probability_of_escalation"])

    work["phase8_priority_score"] = (
        0.24 * minmax(work["dispatch_priority_score"]) +
        0.22 * forecast_pressure +
        0.20 * escalation_pressure +
        0.12 * minmax(work["ccs_score"]) +
        0.10 * minmax(work["growth_pct_change"]) +
        0.08 * minmax(work["criticality_factor"]) +
        0.04 * offender_pressure
    ) * 100.0

    work["tow_readiness"] = (
        work["risk_band"].isin(["Critical", "High"]) |
        (work["future_risk_band"].isin(["Critical", "High"])) |
        (work["probability_of_escalation"] >= 0.5) |
        (work["chronic_vehicle_count_5plus"] >= 1)
    )

    q85 = work["phase8_priority_score"].quantile(0.85)
    q60 = work["phase8_priority_score"].quantile(0.60)
    q35 = work["phase8_priority_score"].quantile(0.35)

    work["patrol_allocation"] = np.select(
        [work["phase8_priority_score"] >= q85, work["phase8_priority_score"] >= q60, work["phase8_priority_score"] >= q35],
        [3, 2, 1],
        default=1,
    ).astype(int)

    work["prescriptive_action"] = np.select(
        [
            work["tow_readiness"],
            work["risk_band"].eq("Critical"),
            work["risk_band"].eq("High"),
        ],
        [
            "Tow standby + patrol",
    ...
    work["priority_band"] = np.select(
        [work["phase8_priority_score"] >= q85, work["phase8_priority_score"] >= q60, work["phase8_priority_score"] >= q35],
        ["Tier 1", "Tier 2", "Tier 3"],
        default="Tier 4",
    )

    shift_window = work.apply(recommended_shift_from_row, axis=1)
    work["recommended_shift"] = [x[0] for x in shift_window]
    work["recommended_time_window"] = [x[1] for x in shift_window]

    work["deployment_note"] = np.where(
        work["probability_of_escalation"] >= 0.5,
        "Likely escalation next week",
        "Monitor for drift",
    )

    offender_actions_df = build_offender_actions(offenders_df)

    work = work.sort_values(
        ["phase8_priority_score", "probability_of_escalation", "predicted_next_week_ccs_score", "ccs_score"],
        ascending=[False, False, False, False],
    ).reset_index(drop=True)
    work["zone_rank"] = np.arange(1, len(work) + 1)

    route_plan_df = build_route_plan(work)

    summary = pd.DataFrame([{
        "planned_hotspots": int(len(work)),
        "tow_ready_hotspots": int(work["tow_readiness"].sum()),
        "critical_hotspots": int(work["risk_band"].eq("Critical").sum()),
        "high_hotspots": int(work["risk_band"].eq("High").sum()),
        "mean_dispatch_priority_score": float(work["dispatch_priority_score"].mean()) if len(work) else np.nan,
        "mean_phase8_priority_score": float(work["phase8_priority_score"].mean()) if len(work) else np.nan,
        "mean_probability_escalation": float(work["probability_of_escalation"].mean()) if len(work) else np.nan,
        "mean_patrol_allocation": float(work["patrol_allocation"].mean()) if len(work) else np.nan,
    }])

    return work, route_plan_df, offender_actions_df, summary
...
def load_precomputed_bundle():
    for phase6_dir in PHASE6_DIRS:
        if phase6_dir.exists():
            ...
            forecast = None
            forecast_src = None
            for phase7_dir in PHASE7_DIRS:
                forecast_src = phase7_dir / "phase7_next_week_forecast_ranked.csv"
                forecast = load_if_exists(forecast_src)
            ...
            if cluster is not None and dispatch is not None:
                bundle = {
                    ...
                    "weekly": weekly,
                    "forecast": ensure_label_column(derive_coords(forecast)) if forecast is not None else None,
                    "forecast_source": str(forecast_src) if forecast is not None else None,
                    "raw": None,
...
forecast_df = bundle.get("forecast")
forecast_src = bundle.get("forecast_source")
raw_df = bundle.get("raw")

for name in ["cluster", "dispatch", "alerts", "offenders", "recommendations", "forecast"]:
    df = bundle.get(name)
...
forecast_df = _merge_coords_from_cluster(forecast_df, cluster_df) if forecast_df is not None else None
...
dispatch_plan_df = compute_dispatch_priority(dispatch_df, forecast_df)
phase8_plan_df, phase8_route_df, phase8_offender_actions_df, phase8_summary_df = build_phase8_engine(dispatch_plan_df, offenders_df)
...
cards = [
    ...
    ("⚠️","Emerging alerts",f"{emerging_count:,}"),
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs(
    ["Executive view", "Interactive map", "Hotspot details", "Alerts", "Offenders & downloads", "Dispatch planner", "Forecasting", "Phase 8 / Prescriptive engine"]
)
    ...
    with right:
        ...
        if forecast_df is not None and len(forecast_df):
            st.download_button(
                ...
                file_name="phase7_next_week_forecast_ranked.csv",
                mime="text/csv",
                use_container_width=True,
            )
...
with tab6:
    ...
    use_forecast = st.checkbox("Blend forecast escalation", value=True)

    planner_base = dispatch_plan_df.copy() if 'dispatch_plan_df' in locals() else table_df.copy()
    if use_emerging and alerts_df is not None and len(alerts_df):
        ...
        if "cluster_label_display" in planner_base.columns:
            planner_base = planner_base.sort_values(["ccs_score", "growth_pct_change"], ascending=[False, False]).drop_duplicates(subset=["cluster_label_display"], keep="first")
        elif "cluster_label" in planner_base.columns:
            planner_base = planner_base.sort_values(["ccs_score", "growth_pct_change"], ascending=[False, False]).drop_duplicates(subset=["cluster_label"], keep="first")

    planner_base = planner_base.sort_values(["dispatch_priority_score" if "dispatch_priority_score" in planner_base.columns else "ccs_score", "growth_pct_change"], ascending=[False, False]).reset_index(drop=True)
    planner_base["dispatch_priority"] = np.arange(1, len(planner_base) + 1)

    plan = planner_base.head(top_units).copy()
    if len(plan):
        time_data = plan.apply(lambda r: pd.Series(recommended_shift_from_row(r), index=["recommended_shift", "recommended_time_window"]), axis=1)
        plan = pd.concat([plan, time_data], axis=1)
        plan["unit_action"] = np.select(
            [plan["risk_band"].eq("Critical"), plan["risk_band"].eq("High")],
            ["Deploy patrol + tow readiness", "Targeted enforcement"],
            default="Monitor",
        )
        plan["deployment_note"] = np.where(
            pd.to_numeric(plan.get("probability_of_escalation", 0.0), errors="coerce").fillna(0.0) >= 0.5,
            "Escalation likely next week",
            "Stable / lower short-term risk",
        )

        st.markdown("**Priority list**")
        st.dataframe(
            plan[[c for c in ["dispatch_priority", "cluster_label_display", "risk_band", "ccs_score", "predicted_next_week_ccs_score", "probability_of_escalation", "delay_minutes_per_vehicle", "recommended_shift", "recommended_time_window", "unit_action", "deployment_note"] if c in plan.columns]],
            use_container_width=True,
            height=340,
        )
        ...
        with c1:
            cov = min(100, round(42 + 5.5 * top_units, 1))
            st.metric("Estimated hotspot coverage", f"{cov}%")
        with c2:
            reduction = min(35, round(3.5 + 1.8 * top_units, 1))
            st.metric("Estimated congestion reduction", f"{reduction}%")
        with c3:
            high_alert = int((plan["risk_band"].isin(["Critical", "High"])).sum())
            st.metric("High-priority deployments", f"{high_alert}")

        fig = px.pie(plan, names="risk_band", title="Risk-band mix in the selected patrol list")
        fig.update_layout(height=340, margin=dict(l=10, r=10, t=35, b=10))
        st.plotly_chart(fig, use_container_width=True)

        window_counts = plan["recommended_time_window"].value_counts().reset_index()
        window_counts.columns = ["time_window", "count"]
        fig = px.bar(window_counts, x="time_window", y="count", title="Recommended time windows")
    ...
    st.markdown(
        ...
        <div class="section-card">
          ...
          <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap: 10px;">
            ...
            <div style="padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:14px;">Forecasting</div>
            <div style="padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:14px;">Patrol Dispatch Planner</div>
            <div style="padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:14px;">Phase 8 Engine</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
# -------------------------
with tab7:
    st.subheader("Next-week forecasting")
    if forecast_df is not None and len(forecast_df):
        forecast_show = forecast_df.copy()
        if "probability_of_escalation" not in forecast_show.columns:
            forecast_show["probability_of_escalation"] = 0.0
        if "predicted_next_week_ccs_score" not in forecast_show.columns:
            forecast_show["predicted_next_week_ccs_score"] = forecast_show.get("ccs_score", 0.0)
        if "current_week_ccs_proxy_score" not in forecast_show.columns:
            forecast_show["current_week_ccs_proxy_score"] = forecast_show.get("ccs_score", 0.0)

        forecast_show["probability_of_escalation"] = pd.to_numeric(forecast_show["probability_of_escalation"], errors="coerce").fillna(0.0)
        forecast_show["predicted_next_week_ccs_score"] = pd.to_numeric(forecast_show["predicted_next_week_ccs_score"], errors="coerce").fillna(0.0)
        forecast_show["current_week_ccs_proxy_score"] = pd.to_numeric(forecast_show["current_week_ccs_proxy_score"], errors="coerce").fillna(0.0)
        forecast_show = forecast_show.sort_values(["predicted_next_week_ccs_score", "probability_of_escalation", "current_week_ccs_proxy_score"], ascending=[False, False, False]).reset_index(drop=True)
        if "forecast_rank" not in forecast_show.columns:
            forecast_show["forecast_rank"] = np.arange(1, len(forecast_show) + 1)

        f1, f2, f3, f4 = st.columns(4)
        with f1:
            st.metric("Forecast rows", f"{len(forecast_show):,}")
        with f2:
            st.metric("Mean escalation prob", safe_metric(forecast_show["probability_of_escalation"].mean(), 3))
        with f3:
            st.metric("Top predicted CCS", safe_metric(forecast_show["predicted_next_week_ccs_score"].max(), 3))
        with f4:
            st.metric("Forecast source", Path(str(forecast_src)).name if forecast_src else "phase7")

        show_cols = [c for c in [
            "forecast_rank", "cluster_key", "cluster_label_display", "future_risk_band",
            "predicted_next_week_ccs_score", "probability_of_escalation",
        ...
        ] if c in forecast_show.columns]
        st.dataframe(forecast_show[show_cols].head(25), use_container_width=True, height=420)

        c1, c2 = st.columns([1, 1])
        with c1:
            label_col = "cluster_label_display" if "cluster_label_display" in forecast_show.columns else "cluster_key"
            topf = forecast_show.head(15).sort_values("predicted_next_week_ccs_score", ascending=True)
            fig = px.bar(topf, x="predicted_next_week_ccs_score", y=label_col, orientation="h",
                         color="future_risk_band" if "future_risk_band" in topf.columns else None,
                         title="Top predicted next-week CCS",
                         labels={"predicted_next_week_ccs_score": "Predicted CCS", label_col: "Hotspot"})
            fig.update_layout(height=420, margin=dict(l=10, r=10, t=35, b=10))
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            fig = px.scatter(forecast_show, x="current_week_ccs_proxy_score", y="predicted_next_week_ccs_score",
                             color="future_risk_band" if "future_risk_band" in forecast_show.columns else None,
                             size="probability_of_escalation", hover_name="cluster_label_display" if "cluster_label_display" in forecast_show.columns else "cluster_key",
                             title="Current vs forecast CCS",
                             labels={"current_week_ccs_proxy_score": "Current CCS", "predicted_next_week_ccs_score": "Predicted CCS"})
            fig.add_trace(go.Scatter(x=[0, 100], y=[0, 100], mode="lines", line=dict(dash="dash"), showlegend=False))
            fig.update_layout(height=420, margin=dict(l=10, r=10, t=35, b=10))
            st.plotly_chart(fig, use_container_width=True)

        st.markdown("**Escalation distribution**")
        fig = px.histogram(forecast_show, x="probability_of_escalation", nbins=20, title="Probability of escalation distribution")
        fig.update_layout(height=300, margin=dict(l=10, r=10, t=35, b=10))
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("Phase 7 forecast file not found. Run Phase 7 to enable this tab.")

# -------------------------
# Tab 8 — Prescriptive Enforcement Engine
# -------------------------
with tab8:
    st.subheader("Prescriptive enforcement engine")
    st.caption("Turns CCS + forecasts into patrol allocation, tow readiness, route sequencing, and offender actions.")

    if phase8_plan_df is None or len(phase8_plan_df) == 0:
        st.info("No prescriptive plan available.")
    else:
        cA, cB = st.columns([1.0, 1.0])
        with cA:
            units = st.slider("Available patrol / tow units", 1, 20, 6, key="phase8_units")
        with cB:
            route_limit = st.slider(
                "Route stops to display",
                3,
                20,
                min(10, max(3, len(phase8_plan_df))),
                key="phase8_route_limit",
            )
        focus_critical = st.checkbox("Prioritize only Critical/High first", value=True, key="phase8_focus_critical")

        plan = phase8_plan_df.copy()
        if focus_critical:
            plan = plan[plan["risk_band"].isin(["Critical", "High"]) | plan["tow_readiness"]].copy()
        plan = plan.sort_values(
            ["phase8_priority_score", "probability_of_escalation", "predicted_next_week_ccs_score"],
            ascending=[False, False, False],
        ).reset_index(drop=True)

        if len(plan) == 0:
            st.warning("No hotspots match the current enforcement filter.")
        else:
            plan = plan.head(units).copy()
            plan["allocated_patrol_units"] = plan["patrol_allocation"].clip(lower=1, upper=3)
            plan["tow_status"] = np.where(plan["tow_readiness"], "Tow standby", "Patrol only")
            plan["action_bundle"] = np.select(
                [
                    plan["risk_band"].eq("Critical"),
                    plan["risk_band"].eq("High"),
                    plan["future_risk_band"].eq("Critical"),
                ],
                [
                    "Immediate blockade / patrol / tow",
                    "Targeted patrol + tow readiness",
                    "Pre-emptive patrol",
                ],
                default="Monitor",
            )

            plan_display_cols = [c for c in [
                "zone_rank", "cluster_label_display", "risk_band", "future_risk_band",
                "phase8_priority_score", "dispatch_priority_score", "probability_of_escalation",
                "predicted_next_week_ccs_score", "allocated_patrol_units", "tow_status",
                "recommended_shift", "recommended_time_window", "prescriptive_action",
                "deployment_note", "action_bundle"
            ] if c in plan.columns]
            st.markdown("**Zone ranking / patrol allocation**")
            st.dataframe(plan[plan_display_cols], use_container_width=True, height=360)

            m1, m2, m3, m4 = st.columns(4)
            with m1:
                st.metric("Allocated zones", f"{len(plan):,}")
            with m2:
                st.metric("Tow-ready zones", f"{int(plan['tow_readiness'].sum()):,}")
            with m3:
                st.metric("Critical / High", f"{int(plan['risk_band'].isin(['Critical', 'High']).sum()):,}")
            with m4:
                st.metric("Mean phase 8 score", safe_metric(plan['phase8_priority_score'].mean(), 3))

            c1, c2 = st.columns(2)
            with c1:
                fig = px.bar(
                    plan.sort_values("phase8_priority_score", ascending=True),
                    x="phase8_priority_score",
                    y="cluster_label_display",
                    orientation="h",
                    color="priority_band" if "priority_band" in plan.columns else None,
                    title="Phase 8 zone ranking",
                    labels={"phase8_priority_score": "Prescriptive score", "cluster_label_display": "Zone"},
                )
                fig.update_layout(height=420, margin=dict(l=10, r=10, t=35, b=10))
                st.plotly_chart(fig, use_container_width=True)
            with c2:
                fig = px.pie(plan, names="tow_status", title="Tow readiness mix")
                fig.update_layout(height=420, margin=dict(l=10, r=10, t=35, b=10))
                st.plotly_chart(fig, use_container_width=True)

            st.markdown("**Recommended time windows**")
            time_counts = plan["recommended_time_window"].value_counts().reset_index()
            time_counts.columns = ["recommended_time_window", "count"]
            if len(time_counts):
                fig = px.bar(time_counts, x="recommended_time_window", y="count", title="Time-window distribution")
                fig.update_layout(height=300, margin=dict(l=10, r=10, t=35, b=10))
                st.plotly_chart(fig, use_container_width=True)

            route_df = phase8_route_df.copy() if phase8_route_df is not None else pd.DataFrame()
            if len(route_df):
                if "cluster_label_display" in route_df.columns:
                    route_df = route_df[route_df["cluster_label_display"].astype(str).isin(plan["cluster_label_display"].astype(str))].copy()
                route_df = route_df.head(route_limit).copy()
                st.markdown("**Route plan**")
                st.dataframe(route_df, use_container_width=True, height=320)

                if "lat" in route_df.columns and "lon" in route_df.columns and route_df[["lat", "lon"]].dropna().shape[0] > 0:
                    route_map = folium.Map(location=BENGALURU_CENTER, zoom_start=11, tiles="cartodbpositron")
                    folium.Marker(
                        location=[BENGALURU_CENTER[0], BENGALURU_CENTER[1]],
                        tooltip="Dispatch depot",
                    ...
                    for _, rr in route_df.iterrows():
                        ...
                        folium.CircleMarker(
                            ...
                            fill=True,
                            fill_opacity=0.9,
                            tooltip=f"{rr.get('route_order', '')}. {clean_text(rr.get('cluster_label_display', ''))}",
                        ).add_to(route_map)
                    if len(coords) >= 2:
                        folium.PolyLine(coords, color="red", weight=4, opacity=0.8).add_to(route_map)
                    route_map = fit_map_bounds(route_map, route_df)
                    st_folium(route_map, use_container_width=True, height=460, key="phase8_route_map")

            st.markdown("**Offender actions**")
            offender_actions = phase8_offender_actions_df.copy() if phase8_offender_actions_df is not None else pd.DataFrame()
            if len(offender_actions):
                st.dataframe(
                    offender_actions[[c for c in [
                        "vehicle_number", "total_violations", "unique_hotspots",
                        "dominant_vehicle_type", "offender_tier", "recommended_action",
                        "watchlist_flag"
                    ] if c in offender_actions.columns]].head(25),
                    use_container_width=True,
                    height=320,
                )
            else:
                st.info("No offender action table available.")

            st.markdown("**Operational summary**")
            if phase8_summary_df is not None and len(phase8_summary_df):
                st.dataframe(phase8_summary_df, use_container_width=True, height=120)

    st.markdown("---")
    st.markdown(
        """
        <div class="section-card">
          <h3>Phase 8 output package</h3>
          <div style="display:grid; grid-template-columns: repeat(2, 1fr); gap: 10px;">
            <div style="padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:14px;">Patrol allocation</div>
            <div style="padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:14px;">Tow readiness</div>
            <div style="padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:14px;">Time-window recommendation</div>
            <div style="padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:14px;">Route plan</div>
            <div style="padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:14px;">Zone prioritization</div>
            <div style="padding:12px; border:1px solid rgba(255,255,255,0.08); border-radius:14px;">Offender actions</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
